...
Usage: python3 timelapse.py <folder> [output.mp4] --interval SECONDS
...
def make_timelapse(folder: Path, output: Path, interval: float, limit: int | None = None) -> None:
    ...
    print(f"Images: {len(entries)}  |  Interval: {interval}s  |  Total: {len(entries) * interval:.1f}s\n")
    ...
    try:
        with concat_file.open('w') as f:
            ...
            for _, img_path in entries:
                ...
                f.write(f"duration {interval:.3f}\n")
...
def main() -> None:
    ...
    make_timelapse(folder, output, args.interval, limit=10 if args.test else None)
