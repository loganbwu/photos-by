...
Pages are driven by partial files in `frontend/partials/`. The build script auto-discovers any file matching `*_partial.html` or `*_partial.md` and generates a page for it.
...
|--------|------|-----------------|
| HTML | `frontend/partials/my_page_partial.html` | Write the `<article>` yourself |
| Markdown | `frontend/partials/my_page_partial.md` | Added automatically as `<article id="myPage">` |
...
A local-only utility for reviewing shots in real time while tethering. As the camera writes CR3 (or JPEG) files to a folder, the viewer automatically detects flash vs. no-flash shots, groups them into series, and composites them with screen blending — the same as the multiple exposure viewer.
